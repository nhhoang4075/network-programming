...
/* Send a message to all ACTIVE clients except the sender */
    ...
    while (1) {
        ...
        if (FD_ISSET(listener, &read_fds)) {
            ...
            if (new_fd < 0) {
                ...
                const char *prompt =
                    "Enter your ID and name (format: client_id: client_name):\n";
        ...
        for (int i = 0; i < num_clients; i++) {
            ...
            if (clients[i].state == REGISTERING) {
                /* Parse "client_id: client_name" */
                char *sep = strstr(buf, ": ");
                if (sep != NULL && sep != buf) {
                    int id_len = sep - buf;
                    char *name_start = sep + 2;

                    /* ID must have no spaces */
                    int valid = (strchr(buf, ' ') == NULL || strchr(buf, ' ') >= sep);
                    /* Name must be non-empty and have no spaces */
